...
agenda_gerar_dadobruto={self.agenda_gerar_dadobruto.checkState(0)}""")
        ...
        try:
	        diaf = self.dateEdit.text().split('/'); dia = diaf[0]; mes = diaf[1]; #print(self.timeEdit.time().toString("H.mm"))
	        horaf = self.timeEdit.time().toString("H.mm").split('.'); hora = horaf[0];minuto = horaf[1];os.system(f'''echo "{minuto} {hora} {mes} {dia} * /usr/bin/python3.8 {path_central}/schedule.py '{path_central}/etc/schedule/scripts/{self.plainTextEdit.toPlainText()}_{data_agenda}-{hora_agenda}.txt'" >> tmp_schedule && crontab tmp_schedule && rm tmp_schedule'''); 
...
class Window2(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def retranslateUi(self, MainWindow):
        ...
        Applies the strings of the buttons/Aplica as strings dos botões.
        """
        MainWindow.setWindowTitle(QCoreApplication.translate("MainWindow", u"Dados", None))
